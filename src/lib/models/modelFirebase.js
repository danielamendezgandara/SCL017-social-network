...
export const authUserWithEmailPassword = (email, password) => {
...
export const addCollection = (newCollection, id, object) => {
  firebase.firestore().collection(newCollection).doc(id).set(object);
};
export const updateDoc = (archive, nameMovie, obj) => {
  firebase.firestore().collection(archive).doc(nameMovie).update(obj);
};
/* Relacionado con los comentarios */
/* Añadir comentarios a cada documento de reseña de la película */
export const addComments = (archive, nameMovie, obj) => firebase.firestore().collection(archive).doc(nameMovie).collection('comments')
  .add(obj);
/* Para obtener los comentarios de la base de datos */
export const getNote = (nameMovie, callback) => firebase.firestore().collection('post').doc(nameMovie).collection('comments')
  .orderBy('date_time', 'desc')
  .limit(3)
  .onSnapshot(callback);
/* Para eliminar los comentarios de la base de datos */
export const deleteComment = (nameMovie, idComment) => firebase.firestore().collection('post').doc(nameMovie).collection('comments')
  .doc(idComment)
  .delete();
